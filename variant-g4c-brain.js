...
// === MUTATION: Multi-projectile dodge (find worst threat) ===
var eProj=false;
var worstThreat=0;
for(var i=0;i<s.scanner.projectile.length;i++){
  var pp=s.scanner.projectile[i];
  var inc=(pp.angleFromRobot+PI)%PI2;
  var threat=Math.abs(angDiff(pp.angle,inc));
  if(pp.range<120&&threat<0.6&&(1/pp.range)>worstThreat){
    eProj=pp;worstThreat=1/pp.range;
  }
}
if(!eProj)eProj=s.scanner.projectile[0]||false;
...
if(!endgame){
  ...
  // === MUTATION: Reduced dodge force to stay closer to center (0.8→0.7) ===
  ...
  if(enemy){
    ...
    if(aimErr<tgtWidth&&since>=30){
      a.desiredLaunch=true;m.lf=m.t;m.ev=-m.ev;m.fired=1;
    }
    if(range<130){
  ...
  if(eProj&&eProj.range<100){
    ...
    if(pDiff<0.5){
      ...
      var dR=Math.abs(angDiff(perpR,toCenter));
      a.desiredAngle=(dL<dR)?perpL:perpR;
      a.desiredForce=s.maxForce*0.6;              // reduced from 0.8
    }
  }
  ...
  if(enemy){
    ...
    if(aimErr2<0.15&&(m.t-m.lf)>=30){
      a.desiredLaunch=true;m.lf=m.t;m.fired=1;
    }
  }
}else{
  if(dCenter>10){
...
if(prox&&prox.entityType==='wall'&&prox.range<35){
  a.desiredAngle=toCenter;a.desiredForce=s.maxForce;
}
