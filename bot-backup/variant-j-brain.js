...
// ULTRA-TIGHT CENTER CONTROL
if(dCenter>120){
  a.desiredAngle=toCenter;
  a.desiredForce=s.maxForce;
  a.desiredScan=120;
}else if(dCenter>40){
  a.desiredAngle=toCenter+m.dir*0.25;
  a.desiredForce=s.maxForce*0.2;
  a.desiredScan=200;
}else{
  a.desiredAngle=myA+m.dir*0.03;
...
if(!endgame){
  ...
  // ENDGAME (15s to 5s): Rush center with dodge
  if(dCenter>15){
    a.desiredAngle=toCenter;
    a.desiredForce=s.maxForce;
  }else{
  ...
  // FINAL STAND (last 5s): Park at dead center, no dodge, no combat
  if(dCenter>10){
    a.desiredAngle=toCenter;
    a.desiredForce=s.maxForce;
  }else{
