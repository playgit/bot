...
// === MUTATION: Health-aware aggression ===
var healthPct=s.health/s.maxHealth*100;
var aggressive=(healthPct>60);
var defensive=(healthPct<25);
...
if(!endgame){
  ...
  // === Health-aware engagement ===
  if(defensive){
    // Low health: only fire if already aimed, don't chase, prioritize center
    if(enemy){
      ...
      // Always pull toward center, never advance
      if(dCenter>20){
    ...
    a.desiredAngle=lA;
    var aimErr=Math.abs(angDiff(myA,lA));
    // === When aggressive, use wider aim tolerance for faster fire rate ===
    var tgtWidth=Math.atan2(aggressive?32:28,range);
    var since=m.t-m.lf;
    if(aimErr<tgtWidth&&since>=30){
      a.desiredLaunch=true;m.lf=m.t;m.ev=-m.ev;m.fired=1;
    }
