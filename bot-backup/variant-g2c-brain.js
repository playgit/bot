...
// === MUTATION: Tighter center control ===
if(dCenter>140){                                   // was 160
  a.desiredAngle=toCenter;
  a.desiredForce=s.maxForce*0.95;                  // was 0.9
  a.desiredScan=120;
}else if(dCenter>45){                              // was 50
  a.desiredAngle=toCenter+m.dir*0.3;
  a.desiredForce=s.maxForce*0.30;                  // was 0.25
  a.desiredScan=180;
}else{
...
if(!endgame){
  ...
  if(enemy){
    ...
    if(range<130){
      ...
      a.desiredForce=s.maxForce*0.5;
    }else if(dCenter>35){                          // was 40
      var dotToCenter=Math.cos(angDiff(lA,toCenter));
      if(dotToCenter>0){
        a.desiredForce=s.maxForce*0.30;            // was 0.25 (stronger pull)
      }else{
        a.desiredForce=-s.maxForce*0.25;           // was 0.2 (stronger pull)
      }
    }else{
      a.desiredForce=s.maxForce*0.03;
    }
