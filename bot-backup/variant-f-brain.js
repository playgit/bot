...
if(!m.t){m.t=0;m.lf=-99;m.ev=1;m.dir=1;m.jit=1;}
m.t++;
...
// MICRO-STRAFE: constant jitter to be harder to hit
if(m.t%12===0)m.jit=-m.jit;
var jitterAngle=m.jit*0.15;

// CENTER CONTROL with micro-strafe
if(dCenter>200){
  a.desiredAngle=toCenter;
  a.desiredForce=s.maxForce*0.8;
  a.desiredScan=120;
}else if(dCenter>80){
  a.desiredAngle=toCenter+m.dir*0.4+jitterAngle;
  a.desiredForce=s.maxForce*0.35;
  a.desiredScan=180;
}else{
  a.desiredAngle=myA+m.dir*0.06+jitterAngle;
  a.desiredForce=s.maxForce*0.08;
  a.desiredScan=220;
...
// ENGAGE with center-pull + micro-strafe
if(enemy){
  ...
  // Add jitter offset when NOT firing to be evasive
  ...
  if(aimErr<tgtWidth&&since>=30){
    ...
    a.desiredAngle=lA+jitterAngle;
  }

  // Center-pull during engagement
  if(range<130){
