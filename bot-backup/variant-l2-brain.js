...
// === MUTATION: Adaptive endgame timing based on health ===
var egTrigger=(s.health>100)?18:(s.health>50)?15:12;
var endgame=(r.remaining<egTrigger);
var finalStand=(r.remaining<6);
...
if(!endgame){
  ...
  if(enemy){
    ...
    // === MUTATION: Rotation-aware lead calculation ===
    var rawLeadAngle=angTo(myX,myY,enemy.x+enemy.speed*Math.cos(enemy.angle)*tF,enemy.y+enemy.speed*Math.sin(enemy.angle)*tF);
    var rotFrames=Math.abs(angDiff(myA,rawLeadAngle))/0.272;
    var totalT=tF+rotFrames/60;
    var pX=enemy.x+enemy.speed*Math.cos(enemy.angle)*totalT;
    var pY=enemy.y+enemy.speed*Math.sin(enemy.angle)*totalT;
  ...
  // === MUTATION: Final stand at 6s instead of 5s (more parking time) ===
  if(dCenter>10){
    a.desiredAngle=toCenter;
    a.desiredForce=s.maxForce;
  }else{
